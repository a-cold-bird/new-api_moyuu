...
func GetPerfMetrics(modelName string, group string, startTs int64, endTs int64) ([]PerfMetric, error) {
	...
	if group != "" {
		query = query.Where(perfMetricGroupCol()+" = ?", group)
	}
...
type PerfMetricSeriesSummary struct {
	ModelName      string `json:"model_name"`
...
func GetPerfMetricsSummaryAll(startTs int64, endTs int64, groups []string) ([]PerfMetricSummary, error) {
	...
	if groups != nil {
		...
		query = query.Where(perfMetricGroupCol()+" IN ?", groups)
	}
	err := query.
		Group("model_name").
...
func GetPerfMetricsSummarySeriesAll(startTs int64, endTs int64, groups []string) ([]PerfMetricSeriesSummary, error) {
	var summaries []PerfMetricSeriesSummary
	query := DB.Model(&PerfMetric{}).
